...
class SMTPHandler(BaseEmailHandler):
    """
    SMTP Handshake Diagnostics: performs deep SMTP – high-confidence results without sending email.
    SMTP handshake – Mail server accepts the recipient (no message sent).
    """
    ...
    async def handle(self, context: PipelineContext) -> PipelineContext:
        ...
        # RFC 5321 expects clients to fall back through the MX list in priority
        # order when the top host is unreachable, rather than giving up after a
        # single host. mx_records is already priority-ordered by the DNS layer.
        mx_hosts = context.mx_records
        ...
        try:
            ...
            return await asyncio.wait_for(
                self._run_smtp_layer(context, mx_hosts, domain, start_time),
                timeout=remaining,
            )
        except asyncio.TimeoutError:
            context.execution_times[self.layer_name] = round((time.perf_counter() - start_time) * 1000, 1)
    ...
    async def _run_smtp_layer(self, context: PipelineContext, mx_hosts: list[str], domain: str, start_time: float) -> PipelineContext:
        """Execute SMTP verification, falling back across the MX list if the
        higher-priority hosts are unreachable/blocked (RFC 5321 fallback)."""
        last_blocked_host: Optional[str] = None
        for idx, mx_host in enumerate(mx_hosts):
            is_last_host = idx == len(mx_hosts) - 1
            result = await self._run_smtp_layer_single_host(
                context, mx_host, domain, start_time, is_last_host=is_last_host
            )
            if result is not None:
                return result
            # This host was blocked/unreachable — try the next one in the list.
            last_blocked_host = mx_host

        # Should be unreachable (the last host always returns a result), but
        # guard defensively anyway.
        context.reasons.append(
            f"All {len(mx_hosts)} MX host(s) were unreachable, starting from '{last_blocked_host}'."
        )
        return await self._uncertain(context, start_time, "blocked")

    async def _run_smtp_layer_single_host(
        self,
        ...
        """Probe one MX host. Returns None (signalling "try the next host") only
        when this host is blocked/unreachable and it isn't the last one to try."""
        ...
        if catchall_status == "blocked":
            if not is_last_host:
                # Don't burn the "uncertain" verdict on a single down host —
                # fall back to the next MX record instead.
                context.reasons.append(
                    f"Port {SMTP_PORT} is blocked or timed out when contacting '{mx_host}'. "
                    f"Falling back to the next MX host."
                )
                return None
            context.smtp_reachable = None
            context.reasons.append(
        ...
        # A dead/unreachable host shouldn't sink the whole verdict to "uncertain"
        # if there's another MX record left to try.
        if result in ("timeout", "blocked_port") and not is_last_host:
            context.reasons.append(
                f"SMTP probe to '{mx_host}' was unreachable ({result}). Falling back to the next MX host."
            )
            return None
    ...
    async def _probe_smtp(mx_host: str, email: str) -> tuple[str, str]:
        ...
        if not PROBE_FROM or not EHLO_HOSTNAME:
            return (
                "error",
                "SMTP_FROM_ADDRESS and SMTP_EHLO_HOST must be configured for mailbox verification.",
            )
