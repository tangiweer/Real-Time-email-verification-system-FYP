...
class BulkJobWorker:
    ...
    async def _process_next_job(self):
        ...
        async with AsyncSessionLocal() as session:
            # Claim with a conditional UPDATE.  A separate SELECT then UPDATE is
            # racy when two application instances poll the same SQLite database.
            ...
            claim = await session.execute(
                update(BulkJob)
                .where(BulkJob.id == candidate.id, BulkJob.status == "PENDING")
            ...
            job = await session.get(BulkJob, candidate.id)
    ...
    async def _requeue_stale_jobs(self) -> None:
        """Recover jobs left PROCESSING by a terminated worker after one hour."""
        cutoff = datetime.utcnow() - timedelta(seconds=STALE_JOB_SECONDS)
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                update(BulkJob)
                .where(BulkJob.status == "PROCESSING", BulkJob.processing_started_at < cutoff)
                .values(status="PENDING", processing_started_at=None)
            )
            await session.commit()
            if result.rowcount:
                logger.warning(f"[worker] Requeued {result.rowcount} stale bulk job(s).")
    ...
    async def _process_csv(self, session: AsyncSession, job: BulkJob):
        ...
        # The bounded queue keeps processing predictable for the FYP deployment.
        emails = []
        ...
        for i in range(0, len(emails), CHUNK_SIZE):
            ...
            for ctx, elapsed_ms in results:
                ...
                # Compare against the enum member, not a raw string. FailedLayer
                # mixes in `str`, so `FailedLayer.ML == "ml"` happens to be True —
                # but relying on that is fragile (e.g. it'd silently break if the
                # enum ever stopped being a str-mixin). Be explicit instead.
