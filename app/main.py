    ...
    return {
        ...
        "layers": [
            ...
            },
            {
                ...
                "description": (
                    "Performs a safe SMTP EHLO/RCPT probe without sending any email. "
                    "Detects catch-all domains via randomised canary pre-probe and "
                    "greylisted servers.  Returns 'uncertain' when results are inconclusive."
                ),
...
async def verify_email(request: Request, email_request: EmailVerifyRequest):
    ...
    await manager.broadcast_verification(response_data.model_dump(mode='json'))
...
async def verify_batch(request: Request, emails: list[str] = Body(...)):
...
async def get_dashboard_analytics(
    ...
    total_result = await db.execute(select(func.count(VerificationLog.id)))
    ...
    ml_saved_result = await db.execute(
        ...
        .where(VerificationLog.ml_short_circuited == True)
    )
