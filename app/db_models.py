...
class User(Base):
    ...
    account_status = Column(String(32), default="ACTIVE", nullable=False, index=True)
...
class BulkJob(Base):
    ...
    # The API key that created this job (see app/core/auth.py). Used for
    # ownership checks on GET /jobs/{id} and /jobs/{id}/download so one
    # caller can't read another caller's results by guessing/reusing a
    # job_id. Nullable to stay compatible with pre-auth rows.
