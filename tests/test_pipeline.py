...
from app.models import PipelineContext, RegistrationRequest, VerificationStatus, FailedLayer
...
class TestRegistrationContracts:
    def test_registration_normalises_email_and_requires_a_strong_password(self):
    ...
    def test_no_click_registration_requires_positive_smtp_verdict(self):
        from app.main import _registration_allowed

        context = make_context("ada@example.com")
        context.smtp_reachable = True
        assert _registration_allowed(context) is True

        context.smtp_reachable = None
        assert _registration_allowed(context) is False
